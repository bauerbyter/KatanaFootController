...
unsigned long dbug_address = 0x00000001;
byte dbug_data = 0x01;
byte dataOut2;  // second byte for 2 byte received data
int bstr_G_type;
int bstr_R_type;
int bstr_Y_type;
int mod_G_type;
    ...
    public://private:
        ...
        bool receive(unsigned long &parameter, byte &dataOut) {
            ...
            if (MS3::RecvData(&rcvd, incoming) == 0) {
                ...
                if (dataLength == 16){ dataOut2 = (byte) data[dataLength - 4];} // second byte for 2 byte received data
                if (dataLength == 29){ bstr_G_type = (byte) data[dataLength - 17]; 
                                      bstr_R_type = (byte) data[dataLength - 16];
                                      bstr_Y_type = (byte) data[dataLength - 15];
                                      mod_G_type = (byte) data[dataLength - 14];
                                      mod_R_type = (byte) data[dataLength - 13];
                                      mod_Y_type = (byte) data[dataLength - 12];
                                      dd1_G_type = (byte) data[dataLength - 11];
                                      dd1_R_type = (byte) data[dataLength - 10];
                                      dd1_Y_type = (byte) data[dataLength - 9];
                                      fx_G_type = (byte) data[dataLength - 8];
                                      fx_R_type = (byte) data[dataLength - 7];
                                      fx_Y_type = (byte) data[dataLength - 6];
        ...
        bool isReady() {
            ...
            if (Usb.getUsbTaskState() == USB_STATE_RUNNING) {
                ...
                MS3_DEBUGLN(F("."));
#ifndef MS3_OVERRIDE_MODE                                                 // debug override, then bypass this                
                MS3::ready = false;             
            }
            return false;
#else 
                MS3::ready = true;                                      // debug override, then return this instead   
            }
            return true;
#endif  
        }
